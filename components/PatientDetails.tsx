...
import { X, Calendar, User, Briefcase, Activity, MapPin, Trash2, Thermometer, AlertTriangle, FileText, HeartPulse } from 'lucide-react';
      ...
      >
        ...
        <div className="relative z-0 pt-10 px-5 pb-5 flex flex-col items-center">
          ...
          <div className="w-full space-y-2">
            <DetailRow icon={<Calendar size={16} />} label="DOB" value={patient.dob} />
            <DetailRow icon={<User size={16} />} label="Rank/Role" value={`${patient.rank || '-'} / ${patient.role || '-'}`} />
            <DetailRow icon={<Briefcase size={16} />} label="Unit" value={patient.unit} />
            <DetailRow icon={<Activity size={16} />} label="Diagnosis" value={patient.diagnosis} highlight />
            <DetailRow icon={<Calendar size={16} />} label="Admitted" value={patient.admissionDate} />
            <DetailRow icon={<MapPin size={16} />} label="Location" value={roomName || 'Unknown'} />
            
            {/* Monitoring Type Selector */}
            <div className={`flex items-center p-2 rounded-xl bg-orange-50 border border-orange-100`}>
              ...
              <div className="flex-1">
                ...
                <select 
                  ...
                  className="mt-1 block w-full bg-white border border-gray-300 text-gray-700 py-1 px-2 rounded leading-tight focus:outline-none focus:border-blue-500 text-xs font-semibold cursor-pointer"
                >
            ...
            {/* Monitor Vitals Toggle */}
            <div className={`flex items-center p-2 rounded-xl bg-red-50 border border-red-100`}>
              <div className="mr-3 text-red-500">
                <HeartPulse size={16} />
              </div>
              <div className="flex-1 flex items-center justify-between">
                <div>
                  <p className="text-[10px] text-gray-500 uppercase font-bold tracking-wider">Mạch & Huyết Áp</p>
                  <p className="text-xs text-gray-700 font-semibold">Theo dõi thêm</p>
                </div>
                <input 
                  type="checkbox" 
          ...
          <div className="w-full mt-4 pt-4 border-t border-gray-100">
             <button 
                ...
                className="w-full bg-green-50 text-green-700 hover:bg-green-100 font-bold py-2.5 rounded-xl border border-green-100 flex items-center justify-center gap-2 transition text-sm"
             >
                <FileText size={16} /> Tạo Giấy Ra Viện
             </button>
          </div>
          ...
          {isConfirming ? (
            <div className="mt-4 w-full p-3 bg-red-50 rounded-xl border border-red-100 animate-fade-in-up">
              ...
              <div className="flex gap-2">
                 <button 
                   ...
                   className="flex-1 py-1.5 bg-white border border-gray-200 text-gray-600 font-semibold rounded-lg hover:bg-gray-50 transition text-xs"
                 >
                   Hủy
                 </button>
                 <button 
                   onClick={(e) => { e.stopPropagation(); onDelete(patient.id); }}
                   className="flex-1 py-1.5 bg-red-500 text-white font-semibold rounded-lg hover:bg-red-600 transition shadow-md shadow-red-200 text-xs"
                 >
                   Xác nhận
                 </button>
              </div>
            ...
            <button 
              ...
              className="mt-4 w-full flex items-center justify-center gap-2 bg-red-50 hover:bg-red-100 text-red-600 font-bold py-2.5 rounded-xl transition border border-red-100 cursor-pointer text-sm"
            >
              <Trash2 size={16} />
              Xóa Bệnh Nhân
            </button>
...
const DetailRow: React.FC<{ icon: React.ReactNode, label: string, value: string, highlight?: boolean }> = ({ icon, label, value, highlight }) => (
  <div className={`flex items-center p-2 rounded-xl ${highlight ? 'bg-red-50 border border-red-100' : 'bg-gray-50'}`}>
    ...
    <div className="flex-1">
      ...
      <p className={`text-xs font-semibold ${highlight ? 'text-red-700' : 'text-gray-700'}`}>{value || 'N/A'}</p>
    </div>
  </div>
);
